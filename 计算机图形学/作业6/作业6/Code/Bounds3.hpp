  ...
  public:
    ...
    inline bool IntersectP(const Ray& ray, const Vector3f& invDir,
                           const std::array<int, 3>& dirisNeg) const {
                            float tMinX = (dirisNeg[0] ? pMax.x : pMin.x - ray.origin.x) * invDir.x;
    float tMaxX = (dirisNeg[0] ? pMin.x : pMax.x - ray.origin.x) * invDir.x;
    // 计算y轴方向交集区间[tMinY, tMaxY]
    float tMinY = (dirisNeg[1] ? pMax.y : pMin.y - ray.origin.y) * invDir.y;
    float tMaxY = (dirisNeg[1] ? pMin.y : pMax.y - ray.origin.y) * invDir.y;
    // 计算z轴方向交集区间[tMinZ, tMaxZ]
    float tMinZ = (dirisNeg[2] ? pMax.z : pMin.z - ray.origin.z) * invDir.z;
    ...
    // 有交集的条件：进入点≤退出点，且退出点>光线最小距离，进入点<光线最大距离
