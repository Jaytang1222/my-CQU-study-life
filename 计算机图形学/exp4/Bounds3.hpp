...
inline bool Bounds3::IntersectP(const Ray& ray, const Vector3f& invDir,
                                const std::array<int, 3>& dirIsNeg) const
{
    // Use the slab method along each axis. dirIsNeg indicates whether the ray
    // is traveling towards the positive or negative direction per axis so that
    // we can pick the correct bound (pMin or pMax) without branching.
    ...
    tMin = std::max(tMin, static_cast<float>(ray.t_min));
    tMax = std::min(tMax, static_cast<float>(ray.t_max));
