...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for (int x = min_x; x <= max_x; ++x) {
        for (int y = min_y; y <= max_y; ++y) {
            ...
            // 计算重心坐标
            auto [alpha, beta, gamma] = computeBarycentric2D(p, t.v);
            
            // 检查是否在三角形内部（带一定容差处理浮点误差）
            if (alpha >= -1e-4 && beta >= -1e-4 && gamma >= -1e-4) {
                // 计算透视校正因子
                float w_reciprocal = 1.0f / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                
                // 计算插值后的视图空间深度
                float Z = w_reciprocal;
                float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                zp *= Z;
                ...
                if (zp < depth_buf[get_index(x, y)]) {
                    ...
                    Eigen::Vector3f interpolated_normal = 
                        (alpha * t.normal[0] / v[0].w() + 
                         ...
                         gamma * t.normal[2] / v[2].w()) * w_reciprocal;
                    ...
                    Eigen::Vector2f interpolated_texcoords = 
                        (alpha * t.tex_coords[0] / v[0].w() + 
                         ...
                         gamma * t.tex_coords[2] / v[2].w()) * w_reciprocal;
