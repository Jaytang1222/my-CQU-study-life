            ...
            default:
            {
                ...
                for (uint32_t i = 0; i < get_lights().size(); ++i)
                {
                    ...
                    else
                    {
                        ...
                        // Shadow test: only occluded if a hit occurs BEFORE the light
                        Intersection shadowIsect = bvh->Intersect(Ray(shadowPointOrig, lightDir));
                        float lightDistance = std::sqrt(lightDistance2);
                        bool inShadow = shadowIsect.happened && shadowIsect.distance < lightDistance - EPSILON;
