...
public:
    MeshTriangle(const std::string& filename)
    {
        ...
        // Center the mesh around origin to appear in the middle of the view
        Vector3f center = (min_vert + max_vert) * 0.5f;
        for (auto &tri : triangles)
        {
            ...
            tri.normal = normalize(crossProduct(tri.e1, tri.e2));
        }

        bounding_box = Bounds3(min_vert - center, max_vert - center);
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (rayTriangleIntersect(v0, v1, v2, ray.origin, ray.direction, tnear, uu, vv))
    {
        inter.happened = true;
        inter.coords = ray.origin + ray.direction * tnear;
        inter.normal = normal;
        if (dotProduct(inter.normal, ray.direction) > 0) inter.normal = -inter.normal;
